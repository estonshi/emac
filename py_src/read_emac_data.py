...
if __name__ == '__main__':
	...
	parser.add_argument("-s", "--showindex", type=str, default="-1", \
		help="only show pattern with index you give, not save to HDF5. For example, '0,1,2' means showing 0,1,2 three patterns.")
	args = parser.parse_args()
	showindex = list(map(int, args.showindex.split(",")))
	showindex.sort()
	...
	if showindex[0] >= 0:
		for i in range(num_data):
			...
			if i not in showindex:
				point = point + all_count
			else:
				print("Show index = %d" % showindex[0])
