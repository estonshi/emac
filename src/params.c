...
bool load_det_info(char* det_file, int* mask, float* det, float* correction){
	// read det file
	FILE* fp;
	fp = fopen(det_file, "r");
	if(fp == NULL){
	...
	__qmax_len = q_max_len;
	__center_q[0] = (q_max_len-1) / 2;
	__center_q[1] = (q_max_len-1) / 2;
	__center_q[2] = (q_max_len-1) / 2;
...
void write_log(uint32 iter_num, float used_time, float rms_change, float beta, uint32 num_rot, float KL_entropy, float prob_min){
	FILE* fp;
	fp = fopen(__log_file, "a");
	fprintf(fp, "%-10u %-12.2f %-12.3e %-12.3e %-12u %-10.5f %-10.8f\n", 
		iter_num, used_time, rms_change, KL_entropy, num_rot, beta, prob_min);
	fclose(fp);
}
...
float read_log(char* read_item, int iter_num){
	...
	if( fp == NULL ){
		printf("[ERROR] The log file is invalid : %s.\n", __log_file);
		return -1;
	}
...
bool load_model(char* model_file, uint32 size, float* model){
	...
	if(fp == NULL){
		printf("[ERROR] Your model file is invalid : %s.\n", model_file);
		return false;
	}
...
bool setup(char* config_file, bool resume, int mpi_rank){
	...
	// load data, the function is from emac_data.h
	...
	// fill photon counts array
	__pix_hasphoton = (int*) malloc(__num_data * 3 * sizeof(int));
	thisp = __dataset;
	for(i=0; i<__num_data; i++){
		__pix_hasphoton[3*i]   = (int)thisp->one_pix + (int)thisp->mul_pix;
		__pix_hasphoton[3*i+1] = (int)thisp->one_pix;
		__pix_hasphoton[3*i+2] = (int)thisp->mul_pix;
		thisp = thisp->next;
	}
	...
	__correction = (float*) malloc(__det_x * __det_y * sizeof(float));
	if( !load_det_info(__det_path, __mask, __det, __correction) ) 
		return false;
	...
	__merge_w = (float*) calloc(__qmax_len*__qmax_len*__qmax_len, sizeof(float));

	// init scaling factor
	__scaling_factor = (float*) malloc(__num_data*sizeof(float));
	thisp = __dataset;
	for(i=0; i<__num_data; i++){
		__scaling_factor[i] = thisp->scale_factor;
		thisp = thisp->next;
	}
	...
	if( !resume ){
		...
		if(init_model[0] != '\0'){
			...
			if( mpi_rank == 0 )
				printf("Using random initial model    : yes\n");
			srand(time(NULL));
			for(i=0; i<__qmax_len*__qmax_len*__qmax_len; i++){
				__model_1[i] = (float) rand() / RAND_MAX * 2 * __dataset_mean_count / (__det_x*__det_y);
			}
		}
	...
	__P_jk = (double*) calloc(__num_data*__quat_num, sizeof(double));
	...
	if( !resume ){
		...
		if( mpi_rank == 0 ){
			...
			mkdir(line, 0750);
			sprintf(line, "%s/merge_w", __output_dir);
			mkdir(line, 0750);
			...
			// write scaling
			sprintf(line, "%s/scaling/scaling_init.txt", __output_dir);
			if(__scale){
				fp = fopen(line, "w");
				for(i=0; i<__num_data; i++){
					fprintf(fp, "%.4f\n", __scaling_factor[i]);
			...
			fprintf(fp, "\n[Iterations]\n");
			fprintf(fp, "%-10s %-12s %-12s %-12s %-12s %-10s %-10s\n", "iter_num", "used_time", "rms_change", "KL_divg", "num_rot", "beta", "prob_LB");
			fclose(fp);
		...
		if(temp <= 0) return false;
			...
			if( __iter_now != 1 && __iter_now % __beta_jump == 0)
...
void free_host_global(){
	...
	free(__P_jk);
	// free scaling
	free(__scaling_factor);
	// free correction
	free(__correction);
	// free pix_hasphoton
	free(__pix_hasphoton);
	// free dataset
	free_dataset();
}
